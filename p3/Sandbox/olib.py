...
def plotData(axis, X, Xerr, Y, Yerr, label=None, capsize=1, elinewidth=0.5, fmt=",", polyfit=1, color="r", xscaleing=1, yscaleing=1, errorbar=True, print_data=True, data_path="data.txt"):
    X = X*xscaleing
    Xerr = Xerr*xscaleing
    Y = Y*yscaleing
    Yerr = Yerr*yscaleing
...
class Table:
    ...
    def saveAsPDF(self, path):
        ...
        string += r"\begin{tabular}{|p{3cm}|p{3cm}|}"+"\n"+r"\hline"+"\n"+r"\multicolumn{2}{|c|}{"+f"{self.title}"+r"}"+r"\\"+"\n"+r"\hline"+"\n"+f"{self.xlabel}&{self.ylabel}"+r"\\"+"\n"+r"\hline"+"\n"
        for i in range(self.X.shape[0]):
            string += f"${self.X[i]}"+r"\pm"+f"{self.Xerr[i]}$"+f"&${self.Y[i]}"+r"\pm "+f"{self.Yerr[i]}$"+r"\\"+"\n"
        string += r"\hline"+"\n"+r"\end{tabular}"+"\n"+r"\end{document}"
