        ...
        for i in range(height):
            for k in range(width):
                if k*height+i<self.X.shape[0]:
                    string += f"${x[k*height+i]}"+r"\pm"+f"{xerr[k*height+i]}$"+f"&${y[k*height+i]}"+r"\pm "+f"{yerr[k*height+i]}$"
                    if(k<width-1):
                        string += "&"
                else:
                    string += "&"
...
def plotData(axis, table, label=None, capsize=1, elinewidth=0.5, fmt=",", polyfit=1, color="r", xscaleing=1, yscaleing=1, errorbar=True, print_data=True, data_path="data.txt"):
    if print_data:
        pass
    if errorbar:
    ...
    if polyfit != 0:
        ...
        plotLine(axis, x*table.x_scaling, y*table.y_scaling, label=label, color=color, linewidth=1)
